...
def DataLoader(file_name,shape,nevts,emax,emin,max_deposit=2,logE=True,norm_data=False):
    rank = hvd.rank()
    size = hvd.size()
...
def ReverseNorm(voxels,e,shape,emax,emin,max_deposit,logE=True,norm_data=False):
...
def polar_to_cart(polar_data,nr=9,nalpha=16,nx=12,ny=12):
    ...
    x_interval = np.linspace(-1,1,nx)
    y_interval = np.linspace(-1,1,ny)
    alpha_pos = np.linspace(1e-5,1,nalpha)
    r_pos = np.linspace(1e-5,1,nr)
    for alpha in range(nalpha):
        for r in range(nr):
            x = r_pos[r] * np.cos(alpha_pos[alpha]*np.pi*2)
            y = r_pos[r] * np.sin(alpha_pos[alpha]*np.pi*2)
            binx = np.argmax(x_interval>x)
            biny = np.argmax(y_interval>y)
            ntotal+=1
